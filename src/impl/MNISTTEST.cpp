...
int main(){
...

  {
    ...
    Mat inputTestData;
    inputTestData.Populate(1, images.data.size(), false);
    inputTestData.Cpy(images.data.data(), images.data.size());

    // Normalize the images 
    const auto scale = 1.0f / 255.0f;
    MatScale(inputTestData, scale);
    ...
    for(size_t i = 0; i < epochs; i++) {
      ...
      for(auto j = 0; j < train_count; j++) {
        ...
        Mat input;
        input.ViewNoAlloc( 1, img_size , inputTestData.data + (j * img_size));
